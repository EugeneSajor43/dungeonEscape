...
public class MiniMax : MonoBehaviour
{
    ...
    public static void PlayerMove()
    {
        ...
        if (GameManager.Instance.GameState != GameState.Heroes2Turn) return;
        Dictionary<Vector2, Tile> map = GridManager.Instance.GetGrid();
        int[][] arr = VectorMap(map);
        
        BaseUnit currentHero = UnitManager.Instance.SelectedHeroes[1];
        ...
        if(UnitManager.Instance.CanEscape == false)
        {
            ...
            if(list.Count == 0) { GameManager.Instance.ChangeState(GameState.Heroes3Turn); }
            ...
            foreach (var move in list)
            {
                ...
                var rs = minimax(move.Item2, depth, alpha, beta, agent = false);
                //print($"rs: {rs}, {alpha}");
                if (rs > alpha)
                {
        ...
        else if(UnitManager.Instance.CanEscape == true)
        {
            ...
            Vector2 pos = new Vector2((int)hero2_Location.x, (int)hero2_Location.y);
        ...
        if (UnitManager.Instance.SelectedEnemy != null && currentHeroBH != null && UnitManager.Instance.SelectedEnemy.transform.position == currentHeroBH.transform.position)
        {
            ...
            GameManager.Instance.Minimax_total_kills++;
